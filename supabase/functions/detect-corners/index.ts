...
function buildFallbackCorners(width: number, height: number): Corners {
...
function parseAnthropicJson(text: string): Record<string, unknown> | null {
...
function readPoint(input: unknown, fallback: CornerPoint): CornerPoint {
  if (!input || typeof input !== "object") {
    return { ...fallback };
  }

  const maybePoint = input as Record<string, unknown>;
  return {
    x: toFiniteNumber(maybePoint.x, fallback.x),
    y: toFiniteNumber(maybePoint.y, fallback.y),
  };
}

function pointArea(corners: Corners): number {
  const polygon = [
    corners.topLeft,
    corners.topRight,
    corners.bottomRight,
...
function normalizeCorners(
  ...
  width: number,
  height: number,
): { valid: boolean; corners: Corners } {
  const fallback = buildFallbackCorners(width, height);
  if (!raw || typeof raw !== "object") {
    return { valid: false, corners: fallback };
  }

  const object = raw.corners && typeof raw.corners === "object"
    ? (raw.corners as Record<string, unknown>)
    : raw;
...
function extractBase64ImageData(image: string): {
  base64Data: string;
  mediaType: string;
} {
...
serve(async (req) => {
  ...
  const image = asText(payload?.image || payload?.imageData);
  const width = normalizeDimension(payload?.width, 1000);
  const height = normalizeDimension(payload?.height, 1000);
...
Return this exact shape:
{
  ...
  "description": "short note"
    ...
    const userPrompt = `Detect the four page corners for perspective correction. Prefer the physical sheet/page boundaries, not text blocks.`;
    ...
    if (!claudeResponse.ok) {
      ...
      throw new Error(
        `Corner detection model request failed (${claudeResponse.status}): ${errorText.slice(
          0,
          240,
        )}`,
      );
    ...
    const aiText = asText(aiResult?.content?.[0]?.text);
    const parsed = parseAnthropicJson(aiText);
    ...
    if (!normalized.valid) {
      return new Response(
        JSON.stringify({
          success: false,
          error: "Automatic corner detection was uncertain. Adjust corners manually.",
          corners: normalized.corners,
          description: asText(parsed?.description) || "Fallback corners returned",
        }),
        {
          status: 200,
          headers: {
    ...
    return new Response(
      JSON.stringify({
        success: true,
        corners: normalized.corners,
        description: asText(parsed?.description) || "Corners detected",
      }),
      {
        status: 200,
        headers: {
    ...
    return new Response(
      JSON.stringify({
        success: false,
        error: error instanceof Error ? error.message : "Failed to detect corners.",
        corners: fallbackCorners,
      }),
      {
        status: 500,
