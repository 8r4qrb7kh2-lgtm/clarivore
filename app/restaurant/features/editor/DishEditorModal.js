...
function DishEditorModal({
  ...
  const deleteDishLabel = asText(overlay?.id || overlay?.name);
  const deleteDishPromptName = deleteDishLabel ? `"${deleteDishLabel}"` : "this dish";

  const handleDeleteDish = () => {
    if (isApplyingIngredientName || !overlay?._editorKey) return;
  ...
  return (
    <>
      ...
      <Modal
        open={editor.dishEditorOpen && showDeleteWarning}
      ...
      >
        <div className="space-y-4">
          <p className="m-0 text-sm text-[#c8d3f5]">
            Are you sure you want to delete{" "}
            <strong className="text-[#ffb3b3]">{deleteDishPromptName}</strong>? This action cannot
            be undone.
          </p>
          <div className="flex justify-end gap-3 max-sm:flex-col-reverse">
            ...
            <Button
              tone="danger"
              size="roomy"
              disabled={isApplyingIngredientName || !overlay?._editorKey}
              className="max-sm:w-full"
