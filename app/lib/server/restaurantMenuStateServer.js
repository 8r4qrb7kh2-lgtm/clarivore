...
export async function fetchRestaurantMenuStateMapFromTables(
  ...
  // The custom pg facade may be backed by one transaction-scoped client.
  // Keep these reads sequential so write transactions don't overlap queries.
  const menuPageRows = await dbClient.restaurant_menu_pages.findMany({
  ...
  const menuDishRows = await dbClient.restaurant_menu_dishes.findMany({
    where: { restaurant_id: { in: ids } },
    select: {
      id: true,
      restaurant_id: true,
  ...
  const ingredientRows = await dbClient.restaurant_menu_ingredient_rows.findMany({
    where: { restaurant_id: { in: ids } },
    select: {
      id: true,
      restaurant_id: true,
  ...
  const brandRows = await dbClient.restaurant_menu_ingredient_brand_items.findMany({
    where: { restaurant_id: { in: ids } },
    select: {
      restaurant_id: true,
